/*
  ...
  1. New Tables
    - `resumes` - Store user resumes with full data structure
      - `id` (uuid, primary key)
      - `user_id` (uuid, foreign key to auth.users)
      - `name` (text, resume name/title)
      - `data` (jsonb, complete resume data)
    ...
    - `templates` - Store resume templates
      ...
      - `data` (jsonb, template resume data)
    ...
    - `analytics` - Store user analytics and events
      - `id` (uuid, primary key)
      - `user_id` (uuid, foreign key to auth.users)
  ...
  2. Security
    ...
    - Analytics allows insert only

  3. Performance
    - Add indexes for common queries
    - Add triggers for automatic updated_at timestamps
    - Add function for automatic user profile creation
...
DO $$
BEGIN
  -- Add foreign key for chat_history.resume_id
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'chat_history_resume_id_fkey' 
    AND table_name = 'chat_history'
  ) THEN
    ALTER TABLE chat_history 
    ADD CONSTRAINT chat_history_resume_id_fkey 
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'resumes_template_id_fkey' 
    AND table_name = 'resumes'
  ) THEN
    ALTER TABLE resumes 
...
CREATE POLICY IF NOT EXISTS "Templates are publicly readable"
  ON templates
  FOR SELECT
  TO authenticated
...
CREATE INDEX IF NOT EXISTS idx_resumes_is_template ON resumes(is_template);
